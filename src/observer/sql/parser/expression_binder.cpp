...
static void wildcard_fields(Table *table, bool include_table_name, vector<unique_ptr<Expression>> &expressions)
{
  ...
  for (int i = table_meta.sys_field_num(); i < field_num; i++) {
    ...
    field_expr->set_name(include_table_name ? string(table->name()) + "." + field.field_name() : field.field_name());
    ...
    unique_ptr<Expression> &expr, vector<unique_ptr<Expression>> &bound_expressions)
{
  ...
  for (Table *table : tables_to_wildcard) {
    wildcard_fields(table, context_.query_tables().size() > 1, bound_expressions);
  }
    ...
    unique_ptr<Expression> &expr, vector<unique_ptr<Expression>> &bound_expressions)
{
  ...
  if (0 == strcmp(field_name, "*")) {
    ...
    FieldExpr *field_expr = new FieldExpr(field);
    field_expr->set_name(context_.query_tables().size() > 1 ? expr->name() : field_name);
